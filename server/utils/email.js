...
// ─── Transport Layer ─────────────────────────────────────────────────────────
// Prefer Resend API (works on Render free tier), fall back to SMTP for local dev
...
function getTransporter() {
  ...
  // For Gmail, use port 465 with true TLS
  return nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 465,
    secure: true, 
...
/**
 * Generic send email function — tries Resend first, falls back to SMTP
 */
async function sendEmail({ to, subject, html, text }) {
  // 1. Try Resend API first (works on Render free tier)
  ...
  if (!transporter) {
    console.log("[Email] No email provider configured (RESEND_API_KEY or SMTP_USER/SMTP_PASS missing)");
    return false;
  }
...
async function sendAccountDeletionEmail(email, name) {
  ...
  try {
    ...
    console.error("[Email] Failed to send deletion email:", err.message);
    return false;
  }
}
